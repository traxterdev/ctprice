<?php
/** admin/posts/toggle.php — publicar/despublicar (reaproveita a coluna `ativo`). */

declare(strict_types=1);

require __DIR__ . '/../../config/bootstrap.php';
require __DIR__ . '/../../includes/AdminAuth.php';
require __DIR__ . '/../../repositories/BlogPostRepository.php';

admin_require_login();

if (($_SERVER['REQUEST_METHOD'] ?? '') !== 'POST' || !admin_verify_csrf($_POST['csrf_token'] ?? null)) {
    header('Location: ' . BASE_URL . '/admin/posts/', true, 302);
    exit;
}

$id = (int) ($_POST['id'] ?? 0);

try {
    $repository = new BlogPostRepository();
    $post = $repository->find($id);
    if ($post) {
        $repository->setActive($id, (int) $post['ativo'] !== 1);
        admin_flash_set('success', 'Status atualizado.');
    } else {
        admin_flash_set('error', 'Post não encontrado.');
    }
} catch (Throwable $e) {
    error_log('CT Price CMS [admin/posts/toggle]: ' . $e->getMessage());
    admin_flash_set('error', 'Não foi possível atualizar agora.');
}

header('Location: ' . BASE_URL . '/admin/posts/', true, 302);
exit;
