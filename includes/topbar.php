    ...
    <div class="topbar__inner">
        <ul class="topbar__contacts">
            ...
            <li class="topbar__item">
                ...
                <span class="notranslate" translate="no"><?= htmlspecialchars($company['telefone_fixo'], ENT_QUOTES, 'UTF-8') ?></span>
            </li>
            <?php endif; ?>
            ...
            <li class="topbar__item">
                <a href="<?= htmlspecialchars($company['whatsapp_principal']['url'], ENT_QUOTES, 'UTF-8') ?>">
                    ...
                    <span class="notranslate" translate="no"><?= htmlspecialchars($company['whatsapp_principal']['numero'], ENT_QUOTES, 'UTF-8') ?></span>
                </a>
            </li>
            <?php endif; ?>
            ...
            <li class="topbar__item">
                <a href="mailto:<?= htmlspecialchars($company['emails']['contato'], ENT_QUOTES, 'UTF-8') ?>">
                    ...
                    <span class="notranslate" translate="no"><?= htmlspecialchars($company['emails']['contato'], ENT_QUOTES, 'UTF-8') ?></span>
                </a>
            </li>
            <?php endif; ?>
        </ul>
        ...
        <?php
        // Seletor de idioma (ajuste pontual, 2026-09-17): as 3 bandeiras já existiam aqui só
        // visualmente — viraram o seletor funcional da tradução automática do site público
        // (GTranslate, ver includes/translate-widget.php + assets/js/language-switcher.js).
        // Cada `<button>` só troca o TEXTO visível da página via `data-lang`; nunca navega,
        // nunca altera href/rota — o clique é interceptado em JS. Português continua a fonte
        // oficial do conteúdo; a bandeira do Brasil restaura o texto original.
        $idiomas = [
        ...
        ?>
        <ul class="topbar__languages notranslate" aria-label="Idiomas disponíveis" translate="no">
            <?php foreach ($idiomas as $idioma): ?>
            <li class="topbar__language">
                <button type="button" class="topbar__language-btn" data-lang="<?= htmlspecialchars($idioma['codigo'], ENT_QUOTES, 'UTF-8') ?>" aria-label="<?= htmlspecialchars($idioma['label'], ENT_QUOTES, 'UTF-8') ?>" title="<?= htmlspecialchars($idioma['label'], ENT_QUOTES, 'UTF-8') ?>" aria-pressed="<?= $idioma['codigo'] === 'pt' ? 'true' : 'false' ?>">
